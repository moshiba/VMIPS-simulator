...
class ALU:
    ...
    def mem_op(self, functionality, instruction):
        ...
        if mem_type == "S":  # scalar
            ...
            # Generalize normal vector load-stores as stride=1
            stride = srf[self.reg_index(
                instruction["operand3"])] if strided else 1

            base_address = srf[self.reg_index(instruction["operand2"])]

            if action == "L":  # load: reg = mem[reg]
                value = vmem[base_address:base_address +
                             vector_length * stride:stride]
                # TODO: Handle vector length and vector mask effects
                vrf[self.reg_index(
                    instruction["operand1"])][:vector_length] = value
            else:  # store: mem[reg] = reg
                value = vrf[self.reg_index(instruction["operand1"])]
                # TODO: Handle vector length and vector mask effects
                vmem[base_address:base_address +
                     vector_length * stride:stride] = value[:vector_length]
